...
export function calculateAddressPermission(permissionKey) {
  const byteArray = asBytes(permissionKey.toString());
  const hexAddress = bytesToHexStr(_hash(Buffer.from(byteArray)));
  const address = `${hashFamilyNamespace()}07${hexAddress.substring(0, 62)}`;
  return address;
}
